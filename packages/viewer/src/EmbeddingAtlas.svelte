...
<script lang="ts">
  ...
  function flyToLocation(lon: number, lat: number) {
    ...
    for (const [id, spec] of Object.entries(get(store.charts))) {
      if (spec.type === "embedding" && spec.data?.isGis) {
        store.updateChartState(id, (draft) => {
          Object.assign(draft, { viewport: { x: lon, y: projectedY, scale: cityScale } });
        });
        break;
      }
    }
  ...
  >
    {#await store.ready then}
      ...
      <div class="m-2 flex flex-row items-center gap-2 flex-wrap">
        ...
        {#if isGis}
          <GeocoderSearch onSelect={flyToLocation} />
        {/if}
