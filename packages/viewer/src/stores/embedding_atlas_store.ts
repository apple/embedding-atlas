...
import { defaultCharts, defaultColumnCharts, defaultPrimaryCharts } from "../charts/default_charts.js";
...
export class EmbeddingAtlasStore {
  ...
  constructor(options: EmbeddingAtlasProps) {
    ...
    const initialize = async () => {
      ...
      const existingCharts = Object.values(initialState.charts ?? {});
      const startsEmpty = existingCharts.length == 0;
      const hasColumnCharts = existingCharts.some(
        (spec: any) => spec?.type === "histogram" || spec?.type === "count-plot",
      );
      const needsColumnDiscovery = startsEmpty || !hasColumnCharts;

      // Mount the query-free primary charts first. On very large datasets,
      // waiting for column cardinality discovery here delays the geospatial
      // scatter's first frame by several seconds.
      if (startsEmpty) {
        try {
          let newCharts = this._defaultPrimaryCharts();
      ...
      if (needsColumnDiscovery) {
        this._scheduleDefaultColumnCharts(get(this.currentLayout));
      }
    };
  ...
  private _defaultPrimaryCharts() {
    const data = this.props.data;
    return defaultPrimaryCharts({
      projection: this._defaultProjection(),
      features: data.features ?? undefined,
      config: this.props.defaultChartsConfig ?? undefined,
    });
  ...
  private _scheduleDefaultColumnCharts(targetLayoutId: string) {
    if (typeof window === "undefined") {
      return;
    }

    const atlasWindow = window as any;
    if (atlasWindow.__atlasDisableColumnDiscovery) {
      return;
    }

    const startedPolling = performance.now();
    const discover = async () => {
      const startedDiscovery = performance.now();
      const data = this.props.data;
      console.log(
        `[atlas-discover] starting column-chart discovery: ${get(this.columns).length} columns, table=${data.table}`,
      );
      try {
        const extra = await defaultColumnCharts({
          coordinator: this.coordinator,
          table: data.table,
        ...
        console.log(
          `[atlas-discover] discovery resolved: ${extra.length} extra charts in ${(performance.now() - startedDiscovery).toFixed(0)}ms`,
        );
        if (extra.length === 0) {
          return;
        }
        this._updateState((draft) => {
          const layoutId = targetLayoutId in draft.layouts ? targetLayoutId : draft.currentLayout;
          const layout = draft.layouts[layoutId];
          if (layout == null) {
            return;
          }
          for (const spec of extra) {
            const chartId = findUnusedId(draft.charts);
            draft.charts[chartId] = spec;
            layout.chartIds ??= [];
            layout.chartIds.push(chartId);
            if (layout.type === "list" && layout.chartsOrder != null) {
              layout.chartsOrder.push(chartId);
            }
          }
        });
      } catch (error) {
        console.warn(
          `[atlas-discover] column-chart discovery FAILED in ${(performance.now() - startedDiscovery).toFixed(0)}ms:`,
          error,
        );
      }
    };
    ...
    const poll = () => {
      const firstBigRender = atlasWindow.__atlasFirstBigRenderGpuLogged === true;
      const overdue = performance.now() - startedPolling > 60_000;
      if (firstBigRender || overdue) {
        console.log(
          `[atlas-discover] poll resolved after ${(performance.now() - startedPolling).toFixed(0)}ms (firstBigRender=${firstBigRender} overdue=${overdue})`,
        );
