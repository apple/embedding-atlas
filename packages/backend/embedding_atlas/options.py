...
class EmbeddingAtlasOptions(TypedDict, total=False):
    ...
    features:
        The column name for features. If specified, a features list view will be created by default.
        The column should be either a list of feature strings, or a list of structs, one per feature,
        each ``{ feature: string, source?: string, index?: int }``.
    ...
    additional_tables:
        Declares auxiliary tables' relation to the main table for cross-table filtering,
        as a mapping ``{name: {"id": ..., "relation": {...}}}``. This sets the ``additionalTables``
        prop; it is separate from loading the table data itself. Each entry's ``id`` is the table's
        row id column, and ``relation`` (optional) declares the join (``mainKey``, ``key``); wrap a
        key in ``UNNEST(...)`` when it is list-valued.
    """
...
def make_embedding_atlas_props(**options: Unpack[EmbeddingAtlasOptions]) -> dict:
    ...
    set_prop("data.features", options.get("features"))
